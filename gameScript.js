//GLOBAL VARIABLES 
const gridSize = 5;
/*gridsize will set game board in 1:1 ratio (5 is default and minimum) using a for loop in the HTML
...
let powerIO = 0; //Sences Power ON/OFF	
let srtstp = 0; //Senses START/STOP button position 	
let didReset = 0; //senses RESET button position
let resetbuttonChecked = 0; //if reset was called from reset button-
...
let timer = 0; //this is the timer let, used for the game timer
let numClicks = 0; //tracks number of user clicks for score keeping and the display
let numLevels = 26; //how many levels are there? this help let generates the levels
let curLevel = 0; //level indicator
let selectORdisplay = 0; //select=0/display=1
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  POWER_BUTTON.addEventListener('mouseup', () => {
    togglePower()
  });
  ...
  SELECT_BUTTON.addEventListener('mouseup', () => {
    openlevelsBox()
  });

  const LEVELS_BG_BUTTON = document.getElementById('levelsBoxbg');
  LEVELS_BG_BUTTON.addEventListener('mouseup', () => {
    hidelevelsBox()
  });
  ...
  const HELP_BG__BUTTON = document.getElementById('helpBoxbg');
  HELP_BG__BUTTON.addEventListener('mouseup', () => {
    hidehelpBox()
  });
...
function togglePower() {
  if (0 == powerIO) { //if POWER is OFF, switch to ON by running these...
    ...
    document.querySelector('#power .Ybutton').style.setProperty('background-color', "#554996"); //set POWER button to PURPLR(ON)
  } else if (1 == powerIO) { //if POWER is already ON, switch OFF by running these...
    createjs.Sound.play('beepdoubleLow'); //sound OFF
    document.querySelector('#power .Ybutton').style.setProperty('background-color', "#FFEA00"); //set POWER button to YELLO(OFF)
    //=====if POWER is switched OFF during game ask first=====//
    if (document.getElementsByClassName('iLight').length >= 1) { //if there are any lights ON, do this...
      ...
      if (1 == didReset) { //if player did Resest the game(1(yes))
        ...
        if (srtstp == 1) { //if START then STOP
          document.querySelector('#start p').innerHTML = 'Start'; //set STOP to START
          document.querySelector('#start .Ybutton').style.setProperty('background-color', "#FFEA00"); //change Start button to YELLOW
...
} //end togglePower function
tapped = 0;
//START/STOP BUTTON
function startStop() {
  if (0 == powerIO) {
    togglePower();
  } //if the Power is still OFF then run Power Toggle to turn it ON 
  if (1 == powerIO) { //if/when the Power is ON...
    ...
    if (0 == srtstp) { //if stop, set START
      ...
      document.querySelector('#start p').innerHTML = 'Stop'; //change START to STOP
      document.querySelector('#start .Ybutton').style.setProperty('background-color', "#554996"); //change button to PURPLE
...
//TOGGLE LIGHT
//click a light and the adjacent lights are also toggled{togglePower/startStop}
function tapLight(obj) {

  let pushedButton = obj;
  // console.log("tapLight", pushedButton);
  ...
  document.querySelector('#clicks .displayOutput').innerHTML = numClicks; //display the increased number of clicks
  //get the row and col of the clicked cell *OBJ is the clicked cell* 
  var row = Number(pushedButton.id.substr(4, 1)); //get row = make it a number(get OBJ ID, skip the first 4 characters, use the next 1 characters)*id=cell(X)x*
  var col = Number(pushedButton.id.substr(5, 1)); //get col = make it a number(get OBJ ID, skip the first 5 characters, use the next 1 characters)*id=cellx(X)*

  //toggle the light of the clicked cell
  if (pushedButton.classList.contains('oLight')) {
    pushedButton.classList.add('iLight');
    pushedButton.classList.remove('oLight');
  } else {
    pushedButton.classList.add('oLight');
    pushedButton.classList.remove('iLight');
  }

  //toggle the light of the cell above the clicked cell (OBJ.id row-1 col)
  let topAdjacentButton = document.getElementById('cell' + (row - 1) + col) ? document.getElementById('cell' + (row - 1) + col) : null;
  if (topAdjacentButton) {
    if (topAdjacentButton.classList.contains('oLight')) {
      topAdjacentButton.classList.add('iLight');
      topAdjacentButton.classList.remove('oLight');
    } else {
      topAdjacentButton.classList.add('oLight');
      topAdjacentButton.classList.remove('iLight');
    }
  }

  //toggle the light of the cell below the clicked cell (OBJ.id row+1 col)
  let bottomAdjacentButton = document.getElementById('cell' + (row + 1) + col) ? document.getElementById('cell' + (row + 1) + col) : null;
  if (bottomAdjacentButton) {
    if (bottomAdjacentButton.classList.contains('oLight')) {
      bottomAdjacentButton.classList.add('iLight');
      bottomAdjacentButton.classList.remove('oLight');
    } else {
      bottomAdjacentButton.classList.add('oLight');
      bottomAdjacentButton.classList.remove('iLight');
    }
  }

  //toggle the light of the cell to the right of the clicked cell (OBJ.id row col+1)
  let rightAdjacentButton = document.getElementById('cell' + row + (col + 1)) ? document.getElementById('cell' + row + (col + 1)) : null;
  if (rightAdjacentButton) {
    if (rightAdjacentButton.classList.contains('oLight')) {
      rightAdjacentButton.classList.add('iLight');
      rightAdjacentButton.classList.remove('oLight');
    } else {
      rightAdjacentButton.classList.add('oLight');
      rightAdjacentButton.classList.remove('iLight');
    }
  }

  ; //toggle the light of the cell to the left of the clicked cell (OBJ.id row col-1)	
  let leftAdjacentButton = document.getElementById('cell' + row + (col - 1)) ? document.getElementById('cell' + row + (col - 1)) : null;
  if (leftAdjacentButton) {
    if (leftAdjacentButton.classList.contains('oLight')) {
      leftAdjacentButton.classList.add('iLight');
      leftAdjacentButton.classList.remove('oLight');
    } else {
      leftAdjacentButton.classList.add('oLight');
      leftAdjacentButton.classList.remove('iLight');
    }
  }

  //checkWin - if the game is START and all lights are OFF - Player Wins Level
  if (1 == srtstp) { //if the game is START
    if (document.getElementsByClassName('iLight').length >= 1) { //AND if 1 or more lights are on - no win yet
...
function setAllLightsOff() {
  ...
  for (let index = 0; index < buttons.length; index++) {
    buttons[index].classList.remove('iLight');
    buttons[index].classList.add('oLight');
  }
}
...
function setAllLightsOn() {
  let buttons = document.getElementsByClassName('button');
  for (let index = 0; index < buttons.length; index++) {
    buttons[index].classList.remove('oLight');
    buttons[index].classList.add('iLight');
  }
}
...
function resetButton() {
  if (0 == powerIO) { //if Power is OFF...
    togglePower(); //run Power Toggle to set Power ON
  }
  ...
  if (1 == srtstp) { //if the game is START do this...
    ...
    if (0 == didReset) { //if didReset is 0 do this...
      ...
      clearBoard();
      document.querySelector('#start p').innerHTML = 'Start'; //change STOP to START
      document.querySelector('#start .Ybutton').style.setProperty('background-color', "#FFEA00"); //change the color of the purple button to YELLOW		
      srtstp = 0;
    }
...
function resetClicks() {
  ...
  document.querySelector('#clicks .displayOutput').innerHTML = numClicks; //display the reset counter
}
//RESET TIMER
function resetTimer() {
  timer = 0; //reset clicks counter back to 0
  document.querySelector('#timer .displayOutput').innerHTML = timer + ':00'; //display the new timer position
}
...
function movetoSelect() {
  ...
  // document.querySelector('#display').style.setProperty('display','none');

  gsap.to("#display", {
    // y: (window.offsetHeight - document.querySelector('#gameBoard').offsetHeight),
    y: 200,
    duration: 1
  });
  gsap.to("#preGame", {
...
function timerCounter() {
  if (1 == srtstp) {
    ...
    document.querySelector('#timer .displayOutput').innerHTML = cvtimer;
...
function levelGenerator(lvl) {
  document.querySelector('#lvlgenBox p#lvlnumber').innerHTML = lvl + 1; //populate display with number of next level
  ...
  if (lvl >= (numLevels + 1)) { //RDM Levels Re-Generator
    ...
    curLevel = lvl;
    console.log("Set Level: ", curLevel);
    setAllLightsOff();
    ...
    document.querySelector('#level .displayOutput').innerHTML = lvl + 1; //fill the level indicator with the appropriate level
...
function RDMlevelGenerator(lvl) {
  ...
  if ((numLevels + 1) == lvl) { //if EASY level
    ...
    document.querySelector('#lvlgenBox p').innerHTML = 'RDM'; //populate display with number of next level
    document.querySelector('#lvlgenBox p#lvlnumber').innerHTML = '2-4'; //populate display with number of next level
  ...
  resetTimer();
  document.querySelector('#level .displayOutput').innerHTML = lvl; //fill the level indicator with the appropriate level
  if (0 == srtstp) {
    startStop();
  }
}
